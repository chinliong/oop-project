...
import com.mygdx.game.GameEngine.Entities.Bin;
import com.mygdx.game.GameEngine.Entities.Entity;
import com.mygdx.game.GameEngine.Entities.Player;
import com.mygdx.game.GameEngine.Entities.PlayerGame;
import com.mygdx.game.GameEngine.Entities.RecyclableType;
import com.mygdx.game.GameEngine.Entities.Recyclables;

public class mytest extends BaseScreen {
    ...
    public mytest(SimulationLifeCycleManager game) {
        super(game);
      ...
      //      game.getEntityManager().addEntity(thrashEntity);
       // }
        ...
        for ( int i = 0; i < generatedCoordinates.size(); i++)
        {
            ...
            switch (thrashImages[i % thrashImages.length]) { // Use mod to avoid IndexOutOfBoundsException
            case "plastic.png":
        	...
        	Recyclables thrashEntity = new Recyclables(thrashImages[i % thrashImages.length], coord[0], coord[1], type);
        	game.getEntityManager().addEntity(thrashEntity);
        }
...
//        game.getBatch().setProjectionMatrix(camera1.camera.combined);
   ...
   //Function to generate random coordinates higher than 200units, and at least 50units apart from each other
    private ArrayList<int[]> generateCoordinates() {
        ...
        while (coordinates.size() < 4) { // Generate until 8 unique coordinates are found
