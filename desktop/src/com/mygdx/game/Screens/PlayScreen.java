...
public class PlayScreen extends BaseScreen {
	...
	// highly becuase in my checkwin() function, i have a condition that checks if
	// the player is out of the window bounds
	...
	// which would then allow the loops happening in the render function to start
	// looping adn then get the AI entity
	...
	public void render(float delta) {
		...
		for (int i = 0; i < game.getEntityManager().getEntities().size(); i++) {
			...
			// game.getAIControlManager().movementRandom((AI)game.getEntityManager().getEntity(AI.class),
			// 3, endLocationX,endLocationY);
			// game.getAIControlManager().getMovementVertical().movementFall((AI)game.getEntityManager().getEntity(AI.class),
			// 3, 600);
			// game.getAIControlManager().getMovementVertical().movementFly((AI)game.getEntityManager().getEntity(AI.class),
			// 1, 0);
			game.getAIControlManager().getMoveHorizontal().moveLeft((AI) game.getEntityManager().getEntity(AI.class), 1,
					800);
			// game.getAIControlManager().getMovementHorizontal().movementRight((AI)game.getEntityManager().getEntity(AI.class),
			// 1, -20);
			...
			if (game.getEntityManager().getEntities().get(i) instanceof Player
				...
				if (game.getInputOutputManager().getInputKeyboard().ifRightPressed() == true) { // right
					game.getPlayerControlManager().walk((Player) game.getEntityManager().getEntities().get(i),
							Keys.RIGHT);
					
				} else if (game.getInputOutputManager().getInputKeyboard().ifLeftPressed() == true) { // left
					game.getPlayerControlManager().walk((Player) game.getEntityManager().getEntities().get(i),
							Keys.LEFT);
					
				} else if (game.getInputOutputManager().getInputKeyboard().ifWPressed() == true) { // w
	...
	public void pauseScreen() { // to pause : esc button
	...
	public void checkForCollision() { // essentially checklose
		// for each AI entity in the list of entities, if its within 20 pixels of the
		// player, the player loses and goes to the lose screen
		int distance = 20;
		for (int i = 0; i < game.getEntityManager().getEntities().size(); i++) { // loop the entity list
			if (game.getEntityManager().getEntities().get(i) instanceof AI) { // if its AI, check for collision
				...
				// abs value of the difference between the player and the AI should be less than
				// 20 to be considered a collision
				if (Math.abs(game.getEntityManager().getEntity(Player.class).getPosX()
						- game.getEntityManager().getEntity(id).getPosX()) < distance
						&& Math.abs(game.getEntityManager().getEntity(Player.class).getPosY()
								- game.getEntityManager().getEntity(id).getPosY()) < distance) {
	...
	public void checkwin() {
		// if the player maange to get out of the window then he wins, goes to main
		// screen
		if (game.getEntityManager().getEntity(Player.class).getPosX() > Gdx.graphics.getWidth()
				|| game.getEntityManager().getEntity(Player.class).getPosY() > Gdx.graphics.getHeight()) {
